...
if [[ ! -f ${only_kubectl} ]]; then
  echo -e "${RED} $only_kubectl not available! ${NC}"
  exit 1
fi

host_kubectl="$only_kubectl  -s http://127.0.0.1:8080 --namespace=firmament-namespace"
...
create_object () {
    ...
    for i in $(eval echo "{$START..$END}");do
        ...
        if [[ "$name" == "$2" ]];then
            echo -e "${GREEN} [PASS] $1 $2 created: ${NC}"
            $host_kubectl get $1
            break
        fi
        if [[ $i -eq $END ]];then
            echo -e "${RED} [FAIL] $1 $2 not created! giving up after $SECONDS seconds.${NC}"
            return 1;
        fi
        printf "."
        sleep 4
...
delete_object () {
    ...
    for i in $(eval echo "{$START..$END}");do
        ...
        if [[ "$name" == "" ]];then
            echo -e "${GREEN} [PASS] $1 $2 is deleted after $SECONDS second!${NC}"
            break
        fi
        if [[ $i -eq $END ]];then
            echo -e "${RED} [FAIL] $1 $2 is not deleted! giving up after $SECONDS seconds.${NC}"
            return 1
        fi
        printf "."
        sleep 2
