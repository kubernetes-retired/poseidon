	...
	/*Describe("Add Job using Poseidon Scheduler", func() {
		...
		Context("using firmament for configuring Job", func() {
			...
			It("should succeed deploying Job using firmament scheduler", func() {
				...
				completions = 2
				_, err = clientset.BatchV1().Jobs(TEST_NS).Create(&v1beta1.Job{
					ObjectMeta: v1.ObjectMeta{
						Name:        name,
						Annotations: annots,
						Labels:      labels,
				...
				By("Waiting 10 seconds")
				time.Sleep(time.Duration(10 * time.Second))
				Job, err := clientset.BatchV1Client.Jobs(TEST_NS).Get(name, meta_v1.GetOptions{})
				Expect(err).NotTo(HaveOccurred())

				Expect(string(int(Job.Status.Active))).To(Equal(string(2)))

				By("Pod was in Running state... Time to delete the Job now...")
				err = clientset.BatchV1Client.Jobs(TEST_NS).Delete(name, &v1.DeleteOptions{})
				Expect(err).NotTo(HaveOccurred())
				By("Waiting 5 seconds")
				time.Sleep(time.Duration(5 * time.Second))
				By("Check for Job deletion")
				_, err = clientset.BatchV1Client.Jobs(TEST_NS).Get(name, meta_v1.GetOptions{})
				if err != nil && errors.IsNotFound(err) {
					//do nothing pod has already been deleted
				}
				Expect("Success").To(Equal("Success"))
