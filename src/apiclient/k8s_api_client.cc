    ...
    if (jval[U("status")].is_null() ||
      ...
      for (auto& iter : jval["nodes"].as_array()) {
        ...
        node_stats.cpu_capacity_ =
          boost::lexical_cast<double>(iter["cpu_capacity"].as_string());
        node_stats.cpu_allocatable_ =
          boost::lexical_cast<double>(iter["cpu_allocatable"].as_string());
        auto& mem_cap = iter["mem_capacity"].as_string();
        node_stats.memory_capacity_kb_ =
          boost::lexical_cast<uint64_t>(mem_cap.substr(0, mem_cap.size() - 2));
        auto& mem_allocatable = iter["mem_allocatable"].as_string();
        node_stats.memory_allocatable_kb_ =
          boost::lexical_cast<uint64_t>(
            mem_allocatable.substr(0, mem_allocatable.size() - 2));
    ...
    if (jval[U("status")].is_null() ||
      ...
      for (auto& iter : jval["pods"].as_array()) {
        ...
        for (auto& iter : pContainerList) {
          ...
          // TODO(ionel): Correctly parse the units.
          cpu_request += boost::lexical_cast<double>(
              container_req.find(U("cpu"))->second.as_string());
          auto& mem_req = container_req.find(U("memory"))->second.as_string();
          memory_request += boost::lexical_cast<uint64_t>(
               mem_req.substr(0, mem_req.size() - 2));
        }
        ...
        pod_stats.memory_request_ = memory_request;
