...
JobDescriptor* SchedulerBridge::CreateJobForPod(const string& pod,
  ...
  // TODO(Eissa): Job descriptor should not be restricted to pod name
  // as a job may be associated with multiple pods.
  jd->set_name(pod);
...
TaskID_t SchedulerBridge::AddPodToJob(const string &pod, const JobID_t &job_id,
                                      JobDescriptor *jd) {
  TaskDescriptor *root_td = jd->mutable_root_task();
